...
- **Unit 2**: Logging in via an authorization provider, and use a token provided by that provider to grant access to features in your service (OAuth)
...
**Authentication** verifies a user's identity, Thinking singing in with your username/password as well as security questions. This is a system trying to verify that a user is who they claim to be.
...
You can think of authentication as someone verifying they have purchased a ticket to a theme part at the will-call booth, and authorization is that same user being given a ride pass wrist band and using that wrist band at all theme park attractions once let inside. Authentication verifies that someone is who they claim to be, while authorization declares what that individual can do once they have verified who they are. Authentication needs to take place before authorization can be provided.
