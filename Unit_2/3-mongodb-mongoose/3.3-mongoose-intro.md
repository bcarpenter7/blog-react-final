...
- `Mixed`: A grab-bag data type, you can put whatever you want in this datatype and Mongoose will not attempt to cast it to any other data type or object. Think of Mixed as a free form field you can place anything you can store in BSON format into.
- `ObjectId`: This is a special class used by mongoDB for storing the object ID of a document. You can think of this a key in a relational database. This object appears to be a string at times but it is in fact an object. Read up a bit on [mongoose ObjectId](https://mongoosejs.com/docs/schematypes.html#objectids)
