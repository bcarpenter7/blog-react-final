...
A common Express applications are architecture is to use a design pattern called MVC(Model, view, Controller) This is a pattern based off of MV*, you can read more on MVC and similar MV* design patterns at [Mozilla Developer Network MVC documentation](https://developer.mozilla.org/en-US/docs/Glossary/MVC). let's put all the templates inside of a folder named `views`:
