...
For example, if container's `flex-direction` is `row` (the default), the:
...
However, in this lesson, we'll look at somewhat simpler option that works well in this case...
